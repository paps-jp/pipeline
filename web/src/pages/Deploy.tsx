...
import { DeployPath, DeployPathCreate, DeployRun, deployApi } from "@/api/client";
import { PageHeader } from "@/components/PageHeader";

const REFRESH_MS = 2000;

function confirmDeletePath(label: string, onConfirm: () => void, t: TFunction) {
  ...
  return (
    <Stack gap="xs">
      ...
      <TextInput
        ...
        placeholder="/home/paps-ai/ai/pipeline/plugins/embed_writer"
      />
      <TextInput
              ...
              <Table.Td>
                {p.last_synced_at ? (
                  <Group gap={4}>
                    ...
                    {p.last_synced_ok === true && <Badge size="xs" color="teal">{t("workers.registry.last_ok")}</Badge>}
                    {p.last_synced_ok === false && <Badge size="xs" color="red">{t("workers.registry.last_ng")}</Badge>}
                  </Group>
              ...
              <Table.Td>
                <Group gap={4}>
                  ...
                  <ActionIcon
                    ...
                    onClick={() => confirmDeletePath(p.label, () => del.mutate(p.id), t)}
                  >×</ActionIcon>
                </Group>
              </Table.Td>
            </Table.Tr>
    ...
    <Stack gap="lg">
      ...
      <Box style={{
        ...
        <Text size="sm" c="dimmed" component="div">
          <strong>① {t("deploy.intro_workers")}</strong> — {t("deploy.intro_workers_desc")} <br/>
